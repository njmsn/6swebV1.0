...
export const InspectionPanel: React.FC<InspectionPanelProps> = ({ isOpen, onToggle, onTaskClick, onViewAllTasks, onSiteClick, onPlaybackToggle }) => {
            ...
            >
              ...
              {isExpanded && (
                <div className="mt-4 border-t border-slate-50 pt-3 px-1 space-y-3.5 animate-in slide-in-from-top-2 duration-300" onClick={(e) => e.stopPropagation()}>
                  <div className="rounded-xl overflow-hidden shadow-sm border border-slate-100/60 aspect-video mb-4"><img src={site.image} className="w-full h-full object-cover transition-transform duration-700 hover:scale-110" /></div>
                  <div className="space-y-3 px-1">
                    <div className="flex items-start"><span className="text-[12px] font-bold text-slate-500 w-24 shrink-0 uppercase tracking-tight">工地名称</span><span className="text-[13px] font-bold text-slate-800">{site.name}</span></div>
                    <div className="flex items-start"><span className="text-[12px] font-bold text-slate-500 w-24 shrink-0 uppercase tracking-tight">工地地址</span><span className="text-[13px] font-medium text-slate-700 leading-relaxed">{site.address}</span></div>
                    <div className="flex items-center"><span className="text-[12px] font-bold text-slate-500 w-24 shrink-0 uppercase tracking-tight">时间</span><span className="text-[13px] font-medium text-slate-700 font-mono tracking-tight">{site.time}</span></div>
                    <div className="flex items-center"><span className="text-[12px] font-bold text-slate-500 w-24 shrink-0 uppercase tracking-tight">负责人</span><div className="flex items-center space-x-2"><div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center text-primary text-[11px] font-black">{site.manager.charAt(0)}</div><span className="text-[13px] font-bold text-slate-800">{site.manager}</span></div></div>
                  </div>
                  <div className="pt-2"><button className="w-full h-10 rounded-xl bg-primary/5 text-primary text-[12px] font-bold hover:bg-primary/10 transition-colors shadow-sm active:scale-[0.98]">查看工地全景</button></div>
                </div>
              )}
            </div>
          );
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center space-x-2 flex-1 min-w-0">
                    <h3 className={`font-bold text-slate-800 truncate shrink-0 ${isDroneRelated ? 'text-[12px] max-w-[180px]' : 'text-[14px] w-[62px]'}`}>
                      {item.name}
                    </h3>
                    {!isDroneRelated && (
                      <div className="w-32 relative h-[14.5px] bg-emerald-50/30 rounded-full overflow-hidden border border-emerald-500/30 shrink-0 transition-colors flex items-center justify-center">
                        <div className="absolute left-0 h-full bg-emerald-400/25 transition-all duration-1000 ease-out" style={{ width: `${item.progress}%` }}></div>
                        <span className="relative text-[10px] font-bold text-emerald-800 leading-none pointer-events-none">{item.progress}%</span>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center space-x-2 shrink-0">
                    {!isDroneRelated && <div className="flex items-center text-[12px] font-bold tracking-tight"><span className="text-[#9a6bff]">1</span><span className="mx-1 text-slate-400 font-normal">-</span><span className="text-[#3b82f6]">35</span></div>}
                    ...
                    <div className="pt-0.5">
                      <div className="flex items-center justify-between mb-3 px-1">
                        ...
                        <div className="flex items-center space-x-2">
                          <div className="relative"><button onClick={() => setSelectedTrajectoryAction(selectedTrajectoryAction === 'analysis' ? null : 'analysis')} className={`peer w-8 h-8 rounded-lg flex items-center justify-center transition-all border active:scale-90 z-20 relative ${selectedTrajectoryAction === 'analysis' ? 'bg-[#9a6bff] text-white shadow-lg shadow-[#9a6bff]/30' : 'bg-white text-slate-500 hover:bg-slate-50'}`}><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 3l8 4.5v9L12 21l-8-4.5v-9L12 3z" strokeWidth="2" /></svg></button><div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 opacity-0 peer-hover:opacity-100 transition-all duration-200 translate-y-2 peer-hover:translate-y-0 z-50 pointer-events-none"><div className="tooltip-bubble tooltip-arrow-bottom text-nowrap">轨迹分析</div></div></div>
                      ...
                      <div className="space-y-4 px-1">{trajectoryData.map((traj, idx) => { const isSelected = selectedTrajectoryKeys.has(`${item.id}-${idx}`); return (<div key={idx} onClick={() => toggleTrajectory(item.id, idx)} className="flex items-center justify-between group/line cursor-pointer select-none"><div className="flex items-center space-x-3.5"><div className={`w-5 h-5 rounded-md border transition-all duration-200 flex items-center justify-center ${isSelected ? 'bg-[#7c4dff] border-[#7c4dff] text-white shadow-sm' : 'bg-white border-slate-200'}`}>{isSelected && <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" strokeWidth="3"/></svg>}</div><div className="flex items-center"><svg className="w-3.5 h-3.5 mr-2.5 text-slate-500 group-hover/line:text-[#9a6bff] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="2"/></svg><span className={`text-[12px] font-normal ${isSelected ? 'text-slate-800' : 'text-slate-700'}`}>{traj.time}</span></div></div><div className="flex items-center space-x-3"><span className={`px-2 py-0.5 rounded text-[12px] font-normal ${isSelected ? 'bg-[#7c4dff]/10 text-[#7c4dff]' : 'bg-slate-50 text-slate-500'}`}>{traj.dist}</span><span className="text-[12px] text-slate-500 font-normal w-12 text-right">{traj.dur}</span></div></div>); })}</div>
                    </div>
                  </div>
                )}
                {activeDetailTab === 'task' && !isDroneRelated && (
                  <div className="space-y-1.5 pt-0 pb-1.5 px-2">
                    <div onClick={() => setIsTaskPlanExpanded(!isTaskPlanExpanded)} className="flex items-center justify-between group cursor-pointer hover:bg-slate-50 py-1 px-2 -mx-2 rounded-lg transition-colors"><span className="text-[12px] font-normal text-slate-700 whitespace-nowrap overflow-hidden text-ellipsis">{isPersonnelRelated ? '用户安检' : '抢修作业'}-{item.name}{selectedDate ? selectedDate.replace(/-/g, '/') : '未选日期'}的计划</span><svg className={`w-4 h-4 text-slate-500 transition-transform duration-300 ${isTaskPlanExpanded ? 'rotate-0' : 'rotate-180'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 15l7-7 7 7" strokeWidth="2"/></svg></div>
                    {isTaskPlanExpanded && (
                      <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        <div className="space-y-0 border-t border-slate-50 pt-0.5 pl-4">{mockTasks.slice(0, 4).map((task) => (<div key={task.id} className="flex items-center justify-between py-1.5 group/task-item border-b border-slate-50/50 last:border-0 pr-1 cursor-pointer hover:bg-slate-50 rounded-md transition-colors" onClick={() => onTaskClick?.(task)}><div className="flex items-center space-x-3 overflow-hidden flex-1"><div className={`w-2 h-2 rounded-full shrink-0 ${task.status === 'completed' ? 'bg-[#10b981]' : 'bg-slate-300'}`}></div><span title={task.address} className="text-[12px] font-normal text-slate-700 truncate group-hover/task-item:text-[#9a6bff] transition-colors">{task.address}</span></div><div className="shrink-0 ml-4"><span className="text-[12px] font-normal text-slate-500 font-mono">{task.time}</span></div></div>))}</div>
                      </div>
                    )}
                  </div>
                )}
                {activeDetailTab === 'alarm' && !isPersonnelRelated && (
                  <div className="pt-2 px-3 space-y-4 pb-3">{mockAlarms.map((alarm) => (<div key={alarm.id} className="flex items-center transition-all group/alarm"><div className="relative w-12 h-12 shrink-0 rounded-none overflow-hidden border border-slate-100 mr-3.5 shadow-sm"><img src={alarm.image} className="w-full h-full object-cover transition-transform duration-500 group-hover/alarm:scale-110" alt="报警图标" /><div className={`absolute top-0 left-0 w-full h-0.5 ${alarm.level === 'high' ? 'bg-rose-500' : alarm.level === 'medium' ? 'bg-amber-500' : 'bg-blue-400'}`}></div></div><div className="flex-1 min-w-0 py-0.5"><div className="flex items-center justify-between mb-1.5"><span title={alarm.type} className="text-[12px] font-bold text-slate-800 truncate max-w-[140px] pr-2 transition-colors group-hover/alarm:text-[#9a6bff]">{alarm.type}</span><span className="text-[12px] text-slate-500 font-mono tabular-nums transition-colors group-hover/alarm:text-slate-700 shrink-0">{alarm.time}</span></div><div className="flex items-center"><div className="flex items-center space-x-1.5"><svg className="w-3.5 h-3.5 text-slate-500 group-hover/alarm:text-slate-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" strokeWidth="2" /></svg><span className="text-[12px] font-normal text-slate-500 tracking-tight uppercase group-hover/alarm:text-slate-700 transition-colors">{isDroneRelated ? '编号: ' : ''}{alarm.plate}</span></div></div></div></div>))}</div>
                )}
                {activeDetailTab === 'log' && (
                  <div className="pt-4 px-5 relative pb-4"><div className="absolute left-[23px] top-5 bottom-3 w-0.5 bg-slate-100 rounded-full"></div><div className="space-y-4">{(isPersonnelRelated ? PERSONNEL_LOGS : VEHICLE_LOGS).map((log) => (<div key={log.id} className="flex items-center relative pl-8 group/log"><div className="absolute left-0 w-3 h-3 rounded-full border-2 bg-white transition-all z-10 border-slate-300 group-hover/log:border-slate-500 group-hover/log:scale-110"></div><div className="flex-1 flex items-center justify-between"><span className={`text-[12px] font-medium transition-colors text-slate-700 group-hover/log:text-slate-900`}>{log.title}</span><span className="text-[12px] text-slate-500 font-medium font-mono tabular-nums leading-none">{log.time}</span></div></div>))}</div></div>
                )}
              </div>
            )}
          </div>
  ...
  return (
    <div className="flex h-full bg-white overflow-visible flex-row">
        ...
        <div className="flex-1 flex flex-col min-h-0 relative">
          {activeSubTab !== 'site' && (
            <div className="px-5 mt-2 space-y-3 relative z-30">
              <div className="flex space-x-3"><div className="relative flex-1"><input type="text" placeholder={activeSubTab === 'personnel' ? "搜索人员..." : isDroneRelated ? "搜索无人机..." : "搜索车辆..."} className="w-full h-10 pl-10 pr-4 bg-slate-50/50 border border-slate-100 rounded-lg text-[12px] font-normal focus:ring-2 focus:ring-[#9a6bff]/20 focus:border-[#9a6bff] outline-none transition-all placeholder:text-slate-500" /><svg className="w-4 h-4 text-slate-500 absolute left-3.5 top-3" fill="none" stroke="currentColor" viewBox="0 0 (24 24)"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg></div><div className="relative" ref={filterRef}><button onClick={() => setIsFilterOpen(!isFilterOpen)} className={`w-10 h-10 flex items-center justify-center border rounded-lg transition-all active:scale-95 shadow-sm ${isFilterOpen ? 'bg-[#9a6bff] border-[#9a6bff] text-white' : 'bg-white border-slate-100 text-slate-600 hover:text-[#9a6bff]'}`}><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg></button>{isFilterOpen && (<div className="absolute top-full right-0 mt-2 w-[280px] bg-white border border-slate-100 rounded-xl shadow-[0_10px_30px_-5px_rgba(0,0,0,0.15)] p-4 space-y-4 z-50 transition-all duration-200 transform origin-top-right scale-100 opacity-100"><div className="flex items-start space-x-4"><span className="text-[12px] font-normal text-slate-500 shrink-0 pt-0.5">状态:</span><div className="flex flex-wrap gap-x-4 gap-y-2.5 flex-1"><FilterCheckbox label="全部" /><FilterCheckbox label="正常" /><FilterCheckbox label="离线" /></div></div></div>)}</div></div>
      ...
      <div className="w-16 h-full border-l border-slate-100 bg-white flex flex-col items-center py-6 shrink-0 relative z-20 overflow-visible"><div className="flex flex-col items-center space-y-4 pb-10"><SidebarButton active={activeSubTab === 'personnel'} onClick={() => handleTabClick('personnel')} label="人员" icon={<path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5s-3 1.34-3 3 1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z" />} /><SidebarButton active={activeSubTab === 'vehicle'} onClick={() => handleTabClick('vehicle')} label="车辆" icon={<path d={VEHICLE_SOLID_PATH} fill="currentColor" />} /><SidebarButton active={activeSubTab === 'drone'} onClick={() => handleTabClick('drone')} label="无人机" icon={<path d={DRONE_PATH} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none" />} /><SidebarButton active={activeSubTab === 'hazard'} isFlashing={unreadTabs.has('hazard') && activeSubTab !== 'hazard'} onClick={() => handleTabClick('hazard')} label="隐患" icon={<path d="M12 2L1 21h22L12 2zm0 3.99L19.53 19H4.47L12 5.99zM11 16h2v2h-2zm0-6h2v4h-2z" />} /><SidebarButton active={activeSubTab === 'alarm'} isFlashing={unreadTabs.has('alarm') && activeSubTab !== 'alarm'} onClick={() => handleTabClick('alarm')} label="报警" icon={<path d="M12 22c1.1 0 2-.9 2-2h-4a2 2 0 002 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />} /><SidebarButton active={activeSubTab === 'site'} isFlashing={unreadTabs.has('site') && activeSubTab !== 'site'} onClick={() => handleTabClick('site')} label="工地" icon={<path d={SITE_ICON_PATH} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />} /><SidebarButton active={activeSubTab === 'droneLeak'} isFlashing={unreadTabs.has('droneLeak') && activeSubTab !== 'droneLeak'} onClick={() => handleTabClick('droneLeak')} label="无人机漏点" icon={<g><path d={DRONE_PATH} stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" fill="none"/><circle cx="12" cy="12" r="2" fill="currentColor" className="opacity-40" /></g>} /><SidebarButton active={activeSubTab === 'vehicleLeak'} isFlashing={unreadTabs.has('vehicleLeak') && activeSubTab !== 'vehicleLeak'} onClick={() => handleTabClick('vehicleLeak')} label="测漏车漏点" icon={<g><path d={TRUCK_SOLID_PATH} fill="currentColor" opacity="0.6" /><circle cx="12" cy="14" r="1.5" fill="white" className="opacity-90" /></g>} /></div><div className="mt-auto space-y-3 pb-2 shrink-0"><div className="w-1 h-1 rounded-full bg-slate-400 mx-auto"></div><div className="w-1 h-1 rounded-full bg-slate-400 mx-auto"></div></div></div>
    </div>
  );
};
...
const SidebarButton: React.FC<{ active: boolean; isFlashing?: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ active, isFlashing, onClick, icon, label }) => (
  <div className="relative group/btn"><button onClick={(e) => { e.stopPropagation(); onClick(); }} className={`w-11 h-11 rounded-lg flex items-center justify-center transition-all duration-300 relative z-10 ${active ? 'bg-[#9a6bff] text-white shadow-lg shadow-[#9a6bff]/30' : isFlashing ? 'animate-alert-flash text-slate-500' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50/80'}`}><svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">{icon}</svg>{isFlashing && !active && <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-rose-500 rounded-full border border-white"></span>}</button><div className="absolute right-full mr-3 top-1/2 -translate-y-1/2 opacity-0 group-hover/btn:opacity-100 transition-all duration-200 translate-x-2 group-hover/btn:translate-x-0 z-[9999] pointer-events-none"><div className="tooltip-bubble relative shadow-2xl">{label}<div className="absolute top-1/2 -translate-y-1/2 -right-[4px] w-2 h-2 bg-[#080b1a] rotate-45"></div></div></div></div>
);
