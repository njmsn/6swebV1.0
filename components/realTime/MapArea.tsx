...
const PlaybackControl: React.FC<{
  ...
  return (
    /* 
       外层定位容器优化：
       1. w-[calc(100%-200px)]: 两侧预留各100px边距，避让左侧按钮(约68px占位)。
       2. max-w-[900px]: 限制最大宽度，防止大屏变形。
       3. 左右间距一致且居中对齐。
    */
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[100] animate-in slide-in-from-bottom-6 duration-500 ease-out w-[calc(100%-200px)] max-w-[900px] min-w-[480px]">
      <div className="bg-white/95 backdrop-blur-md rounded-lg border border-slate-200 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] px-4 py-2.5 flex items-center space-x-4 w-full relative">
        ...
        <div className="flex-1 flex items-center space-x-4">
          ...
          <span className="text-[12px] font-mono text-slate-500 font-bold shrink-0 min-w-[35px] text-right tracking-tighter">
            {Math.floor(progress)}%
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-2 custom-scrollbar">
        {activeTab === '实时' && (
          <div className="animate-in fade-in duration-300">
            <div className="space-y-0"><InfoRow label="职位" value="管网安检员" /><InfoRow label="版本号" value="v6.168.229" /><InfoRow label="IMEI" value="864502041234567" /><InfoRow label="电话" value="15002299778" /><InfoRow label="位置" value="天津市滨海新区泰达大街" /><InfoRow label="状态" value="巡检中 (正常)" /><InfoRow label="上班" value="2025-05-22 08:30:12" /><InfoRow label="下班" value="--" /><InfoRow label="同步" value="2025-05-22 10:45:00" /><InfoRow label="角色" value="普通巡检员" /></div>
            <div className="mt-1.5 pt-1.5 border-t border-slate-50 pb-0.5">
               <div className="flex items-center justify-between px-0.5 space-x-1.5">
                 <div className="w-[68px] relative h-[14px] bg-emerald-50/30 rounded-full overflow-hidden border border-emerald-500/40 shrink-0 transition-colors">
                   <div className="h-full bg-emerald-500/20 transition-all duration-1000 ease-out" style={{ width: `${person.progress || 0}%` }}></div>
                   <div className="absolute inset-0 flex items-center justify-center pointer-events-none"><span className="text-[9px] font-bold text-emerald-700 leading-none">{person.progress || 0}%</span></div>
                 </div>
        ...
        {activeTab === '计划' && (<div className="py-16 flex flex-col items-center justify-center animate-in fade-in duration-500"><div className="w-20 h-20 bg-slate-50 rounded-3xl flex items-center justify-center mb-5 border border-slate-100/50"><svg className="w-10 h-10 text-slate-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg></div><span className="text-[13px] font-normal text-slate-300 tracking-[0.3em] uppercase">暂无计划任务</span></div>)}
  ...
  return (
    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-[0_40px_100px_-20px_rgba(0,0,0,0.25)] border border-slate-100 z-[2000] w-[380px] overflow-hidden flex flex-col animate-in zoom-in-95 fade-in duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-6 min-h-[280px]">
        ...
        {activeTab === '回报' && (<div className="space-y-4">{reportLogs.map(l => (<div key={l.id} className="text-[12px] font-normal leading-relaxed">{l.status} - {l.time}</div>))}</div>)}
      </div>
    </div>
  );
};
...
// 工地详情弹窗组件
const SiteDetailPopup: React.FC<{ site: SiteListItem; onClose: () => void; }> = ({ site, onClose }) => {
  const [activeTab, setActiveTab] = useState('信息');
  const photoGroups = [{ id: 1, url: site.image }, { id: 2, url: 'https://picsum.photos/seed/site-extra/400/300' }];
  ...
  return (
    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-[0_40px_100px_-20px_rgba(0,0,0,0.25)] border border-slate-100 z-[2000] w-[380px] overflow-hidden flex flex-col animate-in zoom-in-95 fade-in duration-300">
      ...
      <div className="px-6 pt-4 pb-1 overflow-x-auto custom-scrollbar">
        <div className="flex space-x-6 border-b border-slate-100 min-w-max">
          {['信息', '历史', '图片', '回报', '检查'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-3 text-[13px] font-bold relative transition-colors ${activeTab === tab ? 'text-primary' : 'text-slate-400 hover:text-slate-600'}`}>
              {tab}
      ...
      <div className="flex-1 overflow-y-auto p-6 min-h-[320px] custom-scrollbar">
        {activeTab === '信息' && (
          <div className="space-y-1 animate-in fade-in duration-300">
            <InfoRow label="工地名称" value={site.name} isBoldValue />
            <InfoRow label="地理位置" value={site.address} />
            <InfoRow label="负责人" value={site.manager} />
            <InfoRow label="巡检时间" value={site.time} />
            <InfoRow label="在线状态" value={site.status === 'online' ? '在线' : '离线'} />
          </div>
        )}
        {activeTab === '历史' && (
          <div className="space-y-4 animate-in fade-in duration-300">
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
              <div className="text-[12px] text-slate-400 mb-1">2025/05/22 08:30</div>
              <div className="text-[13px] text-slate-700 font-medium">张扬完成了例行安全巡检</div>
            </div>
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
              <div className="text-[12px] text-slate-400 mb-1">2025/05/21 14:20</div>
              <div className="text-[13px] text-slate-700 font-medium">系统自动标记该点位为关注工地</div>
            </div>
          </div>
        )}
        {activeTab === '图片' && (
          <div className="grid grid-cols-2 gap-3 animate-in fade-in duration-300">
            {photoGroups.map(p => (
              <img key={p.id} src={p.url} className="w-full h-24 rounded-xl bg-slate-50 object-cover shadow-sm border border-slate-100 hover:scale-105 transition-transform cursor-zoom-in" />
            ))}
          </div>
        )}
        {activeTab === '回报' && (
          <div className="space-y-3 animate-in fade-in duration-300">
            <div className="flex items-center space-x-3 text-[13px] text-slate-600">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
              <span>巡检记录回报正常</span>
            </div>
            <div className="text-[12px] text-slate-400 ml-4 font-mono">回报人：{site.manager} | 时间：{site.time}</div>
          </div>
        )}
        {activeTab === '检查' && (
...
export const MapArea: React.FC<MapAreaProps> = ({ 
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-slate-50">
      ...
      {/* 工地详情弹窗：样式同任务详情 */}
      {selectedSiteDetail && (
        <SiteDetailPopup site={selectedSiteDetail} onClose={() => onSiteClose?.()} />
      )}
