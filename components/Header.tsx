...
export const Header: React.FC = () => {
  ...
  const handleThemeChange = (theme: typeof themes[0]) => {
    ...
    if (theme.isLightSidebar) {
      // 浅色侧边栏设置 - 背景设为主色调的 10% 透明度 (1a)
      const lightBg = `${theme.color}1a`; 
      root.style.setProperty('--sidebar-bg', lightBg);
      root.style.setProperty('--submenu-bg', lightBg);
      
      // 细节微调，确保在有色浅背景下的文字对比度
      ...
      root.style.setProperty('--submenu-item-active', 'rgba(255, 255, 255, 0.6)');
    ...
    <header className="h-16 bg-white border border-slate-100 rounded-xl px-8 flex items-center justify-between shadow-sm shrink-0 relative z-[100]">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="relative group flex items-center h-full">
          <button className="p-2 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-xl transition-all flex items-center justify-center">
        ...
        {/* 通知图标与弹窗 - 层级置顶 */}
        <div className="relative group flex items-center h-full">
          <button className="relative p-2 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-xl transition-all flex items-center justify-center">
              ...
              <div className="flex justify-between px-4 mb-1">
                {themes.map((t) => (
                  <button 
                    ...
                    className="w-6 h-6 rounded shadow-sm border border-black/5 hover:scale-110 active:scale-95 transition-all shrink-0"
